...
pair<long long,int> alphabeta(reversi& pos,int depth=DEPTH,long long a=-1e18,long long b=1e18,bool passed=false){
...
void make_move(reversi& pos,int usecs,int cnt){
    if(cnt>=END_DEPTH){
        cerr << cnt << " " << 1+(cnt&1) << " " << (int)pos.board[64] << "!!!" << endl;
        int x=alphabeta(pos,65).second;
